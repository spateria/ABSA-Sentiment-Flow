...
def ReadProcTrainDataGlobal():
    ...
    #sys.exit(0)
    myCorp = CorpusClass(train_data, train_TXT)
    ...
    for i in range(l):
        ...
        for k in range(dlen):
            ...
            if pol == 'positive' or pol == 'negative' or pol == "neutral":
                t = TextClass(sentence,target,cat,pol,frm,to,idn,prev_pol,prev_idn)
...
def getAllTrainFeats():
    ...
    #############################################################
    of = global_path+'ABSA/savedfeats/only_prevs/featSave.txt'
    of2 = global_path+'ABSA/savedfeats/only_prevs/labelSave.txt'
    f = open(of, 'w')
...
def getAllTrialFeats():
    ...
    #sys.exit(0)
    #############################################################
    of = 'D:/SEMEVAL/FROM_HOME_NEW/ABSA/savedfeats/featSaveTRIAL.txt'
    of2 = 'D:/SEMEVAL/FROM_HOME_NEW/ABSA/savedfeats/labelSaveTRIAL.txt'
    f = open(of, 'w')
...
def predOnTrainData(features, labels, maxent):
    ...
    for x in range(1):
        ...
        cnt = 0;
        for TrainIndices, TestIndices in cross_validation.KFold(n=features.shape[0], n_folds=5, shuffle=False, random_state=None):
            '''l = len(features)
            ...
            '''
            scores2["Accuracy"].append(accuracy_score(Testy_i, ypred_i, average='weighted'))
            scores2["F1"].append(f1_score(Testy_i, ypred_i, average='weighted'))
            scores2["Precision"].append(precision_score(Testy_i, ypred_i, average='weighted'))
            scores2["Recall"].append(recall_score(Testy_i, ypred_i, average='weighted'))
            '''
...
def FitandPredictSVM(sclr, mxnt):
    ...
    of = global_path+'ABSA/savedfeats/only_prevs/featSave.txt.npy'
    of2 = global_path+'ABSA/savedfeats/only_prevs/labelSave.txt.npy'
    f = open(of, 'rb')
    f.seek(0)
    ...
    of = global_path+'ABSA/savedfeats/featSaveGLOBAL.txt.npy'
    f = open(of, 'rb')
    f.seek(0)
    global_features = np.load(f)
    f.close()
    
    '''of = 'D:/SEMEVAL/FROM_HOME_NEW/ABSA/savedfeats/featSaveTRIAL.txt.npy'
    of2 = 'D:/SEMEVAL/FROM_HOME_NEW/ABSA/savedfeats/labelSaveTRIAL.txt.npy'
    f = open(of, 'rb')
    f.seek(0)
    ...
    maxent.append( svm.SVC(C=100, kernel='rbf', class_weight={0: 2, 2: 5}, gamma = 0.001) );
...
#getAllTrainFeats()
#getAllTrialFeats()
#ReadProcTrainDataGlobal()

#FitandPredictSVM(0,0)
#FitandPredictSVM(0,1)

#FitandPredictSVM(1,0)
#FitandPredictSVM(2,0)
'''FitandPredictSVM(0,4)
FitandPredictSVM(0,5)
FitandPredictSVM(0,6)
FitandPredictSVM(0,7)
FitandPredictSVM(0,8)
